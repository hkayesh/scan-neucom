...
class ApproximateMatch_SEQ(Callback):
    ...
    # Arguments
        ...
        pred_dir: path to the directory where this should write the predictions after
            each epoch, which will be scored
    '''
    def __init__(self, valid_toks, valid_y, valid_xs, idx2label, pred_dir='./predictions', fileprefix=''):
        super(Callback, self).__init__()
        self.valid_toks = valid_toks
        self.valid_y = valid_y
        self.valid_xs = valid_xs
        # self.valid_causal_features = valid_xs[1]
        # self.valid_char_features = valid_xs[0]
        self.padlen = self.valid_xs[0].shape[1] if isinstance(self.valid_xs, (list,)) else self.valid_xs.shape[1]
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        ## BIDIRECTIONAL
        pred_probs = self.model.predict(self.valid_xs)
...
def get_approx_match(filename):
    ...
    #     recall = 0.0
    # try:
    #     f1score = 2*precision_adr*recall_adr/(precision_adr+recall_adr)
    # except ZeroDivisionError:
    #     f1score = 0.0
