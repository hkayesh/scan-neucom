...
def map_dict_keys(row):
    ...
    for old_key in old_keys:
        if old_key in mapping.keys():
            mapped_row[mapping[old_key]] = row[old_key]
        else:
            mapped_row[old_key] = row[old_key]

    mapped_row['tgtdrug'] = mapped_row['reldrug']

    if 'meddra_code' in mapped_row.keys():
        mapped_row.pop('meddra_code')
    if 'meddra_term' in mapped_row.keys():
        mapped_row.pop('meddra_term')
...
def create_adr_dataset(t, files, tokset, labelset):
    ...
    for f in ['_'.join([d,t]) for d in files]:
        ...
        with open(os.path.join(Raw_data_dir, t, f), 'rU') as fin:
            ...
            for row in dr:
                # Added for the smm4h dataset
                row = map_dict_keys(row)
...
if __name__ == '__main__':
    ...
    optparser.add_option("-P", "--processeddatadir", dest="processeddatadir", default='./data/seq_labeling/processed', help="Processed dataset directory")
    (opts, _) = optparser.parse_args()

    if opts.validpct:
        assert opts.validpct

    Processed_data_dir = opts.processeddatadir
    ...
    for dtype in trainfiles:
        with open(os.path.join(Processed_data_dir, 'train', dtype+'_train'), 'rU') as fin:
            ...
            for row in rd:
                row['tokens'] = [med_tag if row['tgtdrug'].lower() in x.lower() else x for x in parselist(row['tokens'])]
                row['norm_text'] = [med_tag if row['tgtdrug'].lower() in x.lower() else x for x in parselist(row['norm_text'])]

                if med_tag not in row['tokens']:
                    row['tokens'][0] = med_tag

                t_toks.append(row['tokens'])
                t_lex.append(row['norm_text'])
    ...
    if opts.validpct > 0.0:
        sss = StratifiedShuffleSplit(n_splits=1, test_size=opts.validpct, random_state=0)
        for train_index, test_index in sss.split(t_toks, t_class):
    ...
    for dtype in testfiles:
        with open(os.path.join(Processed_data_dir, 'test', dtype+'_test'), 'rU') as fin:
            rd = csv.DictReader(fin)
            for row in rd:
                row['tokens'] = [med_tag if row['tgtdrug'].lower() in x.lower() else x for x in
                                 parselist(row['tokens'])]
                row['norm_text'] = [med_tag if row['tgtdrug'].lower() in x.lower() else x for x in
                                    parselist(row['norm_text'])]
                if med_tag not in row['tokens']:
                    row['tokens'][0] = med_tag
                test_toks_raw.append(row['tokens'])
                test_lex_raw.append(row['norm_text'])
    ...
    with open(os.path.join(Processed_data_dir, opts.outfile),'wb') as fout:
